...
import com.shatteredicedungeon.effects.BannerSprites.Type;
...
import com.shatteredicedungeon.windows.WndChallenges;
import com.shatteredicedungeon.windows.WndClass;
...
public class StartScene extends PixelScene {
	...
	public void create() {
		...
		curClass = null;
		updateClass( HeroClass.values()[ShatteredIceDungeon.lastClass()] );
	...
	private void startNewGame() {
		...
		if (ShatteredIceDungeon.intro()) {
			ShatteredIceDungeon.intro( false );
	...
	protected void onBackPressed() {
		ShatteredIceDungeon.switchNoFade( TitleScene.class );
	}
	...
	private class ChallengeButton extends Button {
		...
		protected void createChildren() {
			...
			image = Icons.get( ShatteredIceDungeon.challenges() > 0 ? Icons.CHALLENGE_ON :Icons.CHALLENGE_OFF );
			add( image );
		}
		...
		protected void onClick() {
			if (Badges.isUnlocked( Badges.Badge.VICTORY )) {
				StartScene.this.add(new WndChallenges(ShatteredIceDungeon.challenges(), true) {
					public void onBackPressed() {
						super.onBackPressed();
						image.copy( Icons.get( ShatteredIceDungeon.challenges() > 0 ?
