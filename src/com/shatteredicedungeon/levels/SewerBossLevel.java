	...
	protected boolean build() {
		...
		for (int i = 0; i <= 4; i++) {
			...
			if (i < 4) {
				...
				} while (curRoom.type != Type.NULL || distance != 3
						|| curRoom.neigbours.contains(roomEntrance));
		...
		// exit = roomEntrance.top * Level.WIDTH + (roomEntrance.left +
		// roomEntrance.right) / 2;

		// also determines the exat position of the exit
