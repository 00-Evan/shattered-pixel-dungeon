...
public class DeterminismIntegrationTest {
    ...
    public void testIntegration_hero_moves_fights_saves_then_checks_determinism() {
        // ===== SETUP: Prepare external commands =====
        List<List<GameCommand>> allCommandsPerTurn = prepareCommandScript();

        // ===== PATH A: Direct-run (baseline) =====
        System.out.println("Running Path A: Direct-run (baseline)...");
        TurnHashSequence baselineSeq = executeDirectRun(allCommandsPerTurn);
        EngineContext baselineContext = captureBaselineState(allCommandsPerTurn);

        // ===== PATH B: Direct-run with mid-run save/load =====
        System.out.println("Running Path B: Direct-run with save/load...");
        TurnHashSequence saveLoadSeq = executeSaveLoadRun(allCommandsPerTurn);
        EngineContext saveLoadContext = captureSaveLoadState(allCommandsPerTurn);

        // ===== PATH C: Replay-run =====
        System.out.println("Running Path C: Replay-run...");
        TurnHashSequence replaySeq = executeReplayRun(allCommandsPerTurn);
        EngineContext replayContext = captureReplayState(allCommandsPerTurn);
    ...
    /**
     * Prepare the command script for all 12 turns.
     *
     * This is the "input tape" that will be used by all three execution paths.
     *
     ...
     *
     * @return List of commands per turn
     */
    private List<List<GameCommand>> prepareCommandScript() {
        List<List<GameCommand>> commandsPerTurn = new ArrayList<>();

        // We'll need to know the hero ID, but we can't know it yet
        // So we'll use a placeholder and fill it in during execution
        // For now, just indicate the intent of each turn

        // Turns 0-4: MOVE RIGHT (5 times)
        for (int i = 0; i < 5; i++) {
            commandsPerTurn.add(new ArrayList<>()); // Placeholder, will be filled
        }

        // Turns 5-8: MOVE RIGHT (4 more times)
        for (int i = 5; i < 9; i++) {
            commandsPerTurn.add(new ArrayList<>());
        }

        // Turns 9-11: ATTACK (3 times)
        for (int i = 9; i < 12; i++) {
            commandsPerTurn.add(new ArrayList<>());
        }
    ...
    /**
     * Fill command script with actual commands for a specific hero.
     *
     * @param heroId Hero actor ID
     * @param mobId Mob actor ID (for attacks)
     * @param commandsPerTurn Command list to fill
     */
    private void fillCommandScript(ActorId heroId, ActorId mobId, List<List<GameCommand>> commandsPerTurn) {
        // Turns 0-8: MOVE RIGHT
        for (int turn = 0; turn < 9; turn++) {
            Point targetPos = new Point(2 + turn + 1, 2); // Start at (2,2), move right
            commandsPerTurn.get(turn).clear();
            commandsPerTurn.get(turn).add(GameCommand.move(heroId, targetPos));
        }
        ...
        for (int turn = 9; turn < 12; turn++) {
            commandsPerTurn.get(turn).clear();
            if (mobId != null) {
                commandsPerTurn.get(turn).add(GameCommand.attack(heroId, mobId));
            }
        }
    }
    ...
    private TurnHashSequence executeDirectRun(List<List<GameCommand>> commandsPerTurn) {
        ...
        fillCommandScript(heroId, mobId, commandsPerTurn);

        List<TurnHashRecord> hashes = new ArrayList<>();

        for (int turn = 0; turn < TOTAL_TURNS; turn++) {
            // Get current mob ID (may have died)
            ActorId currentMobId = findMobId(context);
    ...
    private EngineContext captureBaselineState(List<List<GameCommand>> commandsPerTurn) {
    ...
    private TurnHashSequence executeSaveLoadRun(List<List<GameCommand>> commandsPerTurn) {
        ...
        // Update command script for loaded context
        fillCommandScript(loadedHeroId, loadedMobId, commandsPerTurn);
    ...
    private EngineContext captureSaveLoadState(List<List<GameCommand>> commandsPerTurn) {
        // Re-run to capture final state
        EngineFactory factory = this::createIntegrationEngine;
        EngineContextWithEngine pair = factory.create(TEST_SEED);
        EngineContext context = pair.getContext();
    ...
    private TurnHashSequence executeReplayRun(List<List<GameCommand>> commandsPerTurn) {
        EngineFactory factory = this::createIntegrationEngine;
    ...
    private EngineContext captureReplayState(List<List<GameCommand>> commandsPerTurn) {
        EngineFactory factory = this::createIntegrationEngine;
