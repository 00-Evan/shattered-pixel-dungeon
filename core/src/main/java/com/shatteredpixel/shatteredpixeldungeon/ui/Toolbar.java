...
public class Toolbar extends Component {
	...
	public static int QUICKSLOT_COUNT = 8;

	@Override
	protected void createChildren() {

		btnQuick = new QuickslotTool[QUICKSLOT_COUNT];
	...
	protected void layout() {
		...
		for(int i = 0; i < QUICKSLOT_COUNT; i++) {
			if (i == 0 && !SPDSettings.flipToolbar() ||
				i == QUICKSLOT_COUNT - 1 && SPDSettings.flipToolbar()){
		...
		switch(Mode.valueOf(SPDSettings.toolbarMode())){
			case SPLIT:
				...
				if (btnQuick[QUICKSLOT_COUNT - 1].left() < btnSearch.right()){
					float diff = Math.round(btnSearch.right() - btnQuick[QUICKSLOT_COUNT - 1].left())/2;
					for( int i = 0; i < QUICKSLOT_COUNT; i++){
			...
			case GROUP:
				...
				if (btnQuick[QUICKSLOT_COUNT - 1].left() < 0){
					float diff = -Math.round(btnQuick[QUICKSLOT_COUNT - 1].left())/2;
					for( int i = 0; i < QUICKSLOT_COUNT; i++){
						btnQuick[i].setPos( btnQuick[i].left()+diff, btnQuick[i].top() );
					}
				}
