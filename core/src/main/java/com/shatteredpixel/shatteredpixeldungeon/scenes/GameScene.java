...
import com.right.helveticpixeldungeon.windows.WndInfoFacility;
...
public class GameScene extends PixelScene {
    ...
    public static void add(Facility facility) {
        facility.setLevel(Dungeon.level);
        Dungeon.level.facilities.put(facility.getPos(), facility);
        if (scene != null) {
            scene.addFacilitySprite(facility);
        }
    ...
    private static ArrayList<Object> getObjectsAtCell(int cell) {
        ...
        Facility facility = Dungeon.level.facilities.get(cell);
        if (facility != null && facility.seen) objects.add(facility);
    ...
    private static ArrayList<String> getObjectNames(ArrayList<Object> objects) {
        ...
        for (Object obj : objects) {
            ...
            else if (obj instanceof Facility) names.add(Messages.titleCase(((Facility) obj).name()));
        }
        return names;
    }
    ...
    public static void examineObject(Object o) {
        if (o == Dungeon.hero) {
            ...
            GameScene.show(new WndInfoFacility((Facility) o));
        } else {
    ...
    public void create() {
        ...
        facilities = new Group();
        add(facilities);
        for (Facility facility : Dungeon.level.facilities.valueList()) {
...
//			}
    ...
    /**
     * @param facility {@code facility.sprite must be not null}
     */
    private void addFacilitySprite(Facility facility) {
        FacilitySprite sprite = facility.getSprite();
        if (sprite != null) {
            sprite.visible = true;
            facilities.add(sprite);
            sprite.link(facility);
        }
    ...
    private static final CellSelector.Listener defaultCellListener = new CellSelector.Listener() {
        ...
        public void onRightClick(Integer cell) {
            ...
            if (objects.isEmpty()) {
                ...
                title = textLines.remove(0);
                image = TerrainFeaturesTilemap.tile(cell, Dungeon.level.map[cell]);
            } else if (objects.get(0) instanceof Facility facility) {
                title = textLines.remove(0);
                image = facility.makeSprite();
            }
            ...
            if (objects.isEmpty()) {
                ...
                textLines.add(0, Messages.get(GameScene.class, "interact"));
            } else if (objects.get(0) instanceof Facility) {
                textLines.add(0, Messages.get(GameScene.class, "interact"));
            }
