...
//			}
    ...
    public boolean act() {
        ...
        if (curAction == null) {
            ...
            actResult = switch (curAction) {
                case HeroAction.Move move -> actMove(move);
                case HeroAction.Interact interact -> actInteract(interact);
                case HeroAction.Buy buy -> actBuy(buy);
                case HeroAction.PickUp pickUp -> actPickUp(pickUp);
                case HeroAction.OpenChest openChest -> actOpenChest(openChest);
                case HeroAction.Unlock unlock -> actUnlock(unlock);
                case HeroAction.Mine mine -> actMine(mine);
                case HeroAction.LvlTransition lvlTransition -> actTransition(lvlTransition);
                case HeroAction.Attack attack -> actAttack(attack);
                case HeroAction.Alchemy alchemy -> actAlchemy(alchemy);
                case HeroAction.Operate operate -> actOperate(operate);
                default -> false;
            };
        }
    ...
    private boolean actOperate(HeroAction.Operate action) {
        boolean isAdjacent = Dungeon.level.adjacent(pos, action.dst);
        if (isAdjacent) {
            path = null;
        }
...
//		}
    ...
    public boolean handle(int cell) {
        ...
        }else if (fieldOfView[cell] && facility != null) {
            curAction = new HeroAction.Operate(facility);
