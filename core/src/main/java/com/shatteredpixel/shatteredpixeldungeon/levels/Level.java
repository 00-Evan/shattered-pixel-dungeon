...
public abstract class Level implements Bundlable {
    ...
    public SparseArray<Facility> facilities;
    ...
    protected ArrayList<Facility> facilitiesToSpawn = new ArrayList<>();
...
//		}
        ...
        collection = bundle.getCollection(FACILITIES);
        for (Bundlable f : collection) {
            Facility facility = (Facility) f;
            facility.setLevel(this);
            facilities.put(facility.getPos(), facility);
        }
    ...
    public void storeInBundle(Bundle bundle) {
        ...
        bundle.put(FACILITIES, facilities.valueList());
    ...
    protected void creatFacilities() {
    }
    ...
    public void addFacilityToSpawn(Facility facility) {
        if (facility != null) {
            facilitiesToSpawn.add(facility);
        }
    }
    ...
    public Heap drop(Item item, int cell) {
        ...
        Facility facility=facilities.get(cell);
        if(facility!=null){
            return facility.onDropItem(heap);
        }
    ...
    public Facility setFacility(Facility facility, int x, int y) {
        return setFacility(facility, x + y * width());
    }

    public Facility setFacility(Facility facility, int pos) {
        Facility existingFacility = facilities.get(pos);
        if (existingFacility != null) {
            facilities.remove(pos);
            existingFacility.onRemoveFromLevel();
        }
        facility.setLevel(this);

        facility.setPos(pos);
        this.passable[pos] = (facility.type() & Facility.PASSABLE) != 0;
        this.flamable[pos] = (facility.type() & Facility.FLAMABLE) != 0;
        facilities.put(pos, facility);
        GameScene.updateMap(pos);
        return facility;
    }
    ...
    public void updateFieldOfView(Char c, boolean[] fieldOfView) {
        ...
        if (sighted) {
            ...
            //cannot see through some OPAQUE facility
            if(!this.facilities.isEmpty()){
                for(Facility f : facilities.values()){
                    if((f.type()&Facility.OPAQUE)!=0&&fieldOfView[f.getPos()]){
                        if (blocking == null) {
                            System.arraycopy(Dungeon.level.losBlocking, 0, modifiableBlocking, 0, modifiableBlocking.length);
                            blocking = modifiableBlocking;
                        }
                        blocking[f.getPos()] = true;
                    }
                }
            }
        ...
        if (c.isAlive() && c == Dungeon.hero) {
            ...
            if (c.buff(MindVision.class) != null) {
                for (Mob mob : mobs) {
                    ...
                    if(Char.hasProp(mob,Property.NOMIND)){
                        continue;
                    }
//                    for (int i : PathFinder.NEIGHBOURS9) {
...
//                    for (int i : PathFinder.NEIGHBOURS9) heroMindFov[p + i] = true;
        ...
        if (c == Dungeon.hero) {
            ...
            for (Facility facility : facilities.valueList()) {
                if (!facility.seen && fieldOfView[facility.getPos()]) {
                    facility.seen = true;
                }
            }
        }
