...
public class WallOcclusionTilemap extends Tilemap {
	...
	public WallOcclusionTilemap() {
		super(Assets.Environment.OCCLUSION_SHADOWS, new TextureFilm( Assets.Environment.OCCLUSION_SHADOWS, SIZE, SIZE ) );
	...
	public synchronized void updateMapCell(int cell) {
		...
		if (Dungeon.level.insideMap(cell)) {
			for (int i : PathFinder.NEIGHBOURS9) {
				data[cell + i] = getTileVisual(cell + i);
			}
	...
	private int getTileVisual( int cell ){
		int curr = EMPTY;
		if (door(cell)){

			if (wall(cell-1) && wall(cell+1)){
				curr = DOOR_HORIZ;
			} else {
				curr = DOOR_VERT;
			}
			...
			if (wall(cell-mapWidth) && Dungeon.level.map[cell] != Terrain.ALCHEMY) {
				curr += WALL_ABOVE;

				//left
				if (wall(cell-1)){
					curr += WALL_LEFT;
				} else if (wall(cell+mapWidth-1)){
