...
public class SoulParticle extends PixelParticle.Shrinking {
	
	public static final Emitter.Factory MISSILE = new Factory() {
		@Override
		public void emit( Emitter emitter, int index, float x, float y ) {
			((SoulParticle)emitter.recycle( SoulParticle.class )).reset( x, y );
		}
	};
	...
	public static final Emitter.Factory CURSE = new Factory() {
		@Override
		public void emit( Emitter emitter, int index, float x, float y ) {
			((SoulParticle)emitter.recycle( SoulParticle.class )).resetCurse( x, y );
		}
	};
	...
	public static final Emitter.Factory UP = new Factory() {
		@Override
		public void emit( Emitter emitter, int index, float x, float y ) {
			((SoulParticle)emitter.recycle( SoulParticle.class )).resetUp( x, y );
		}
	};
	...
	public void update() {
		...
		color( ColorMath.interpolate( 0xFFFFFF, 0xCC99FFFF, p ) );
