...
public class FloatingText extends RenderedTextBlock {
    ...
    // !
    public static int EXCLAMATION = 27;
    // ?
    public static int QUESTION = 28;
    private Image icon;
    ...
    public FloatingText() {
        ...
        setHightlighting(/*false*/true);
    }

    public static void show(float x, float y, String text, int color) {
        show(x, y, -1, text, color, -1, false, false);
    }

    public static void show(float x, float y, int key, String text, int color) {
        show(x, y, key, text, color, -1, false, false);
    }

    public static void showWithScale(float x, float y, int key, String text, int color) {
        show(x, y, key, text, color, -1, false, true);
    }

    public static void show(float x, float y, int key, String text, int color, int iconIdx, boolean left, boolean shouldScale) {
        Game.runOnRenderThread(new Callback() {
            @Override
            public void call() {
                FloatingText txt = GameScene.status();
                if (txt != null) {
                    txt.reset(x, y, text, color, iconIdx, left, shouldScale);
    ...
    public void update() {
        ...
        if (timeLeft >= 0) {
            if ((timeLeft -= Game.elapsed) <= 0) {
                ...
                if (shouldScale) {
                    float scale = 1 - timeLeft / LIFESPAN;
                    scale = (float) (1 + 0.75 * (-Math.cos(scale * Math.PI) / 2 + .5));

                    zoom(scale / (float) PixelScene.defaultZoom);
    ...
    public void reset(float x, float y, String text, int color, int iconIdx, boolean left, boolean shouldScale) {
