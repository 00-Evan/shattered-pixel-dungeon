...
public class GoldenParticle extends PixelParticle {
	//황금화 필드 이펙트

	public static final Emitter.Factory FACTORY = new Emitter.Factory() {
		@Override
		public void emit( Emitter emitter, int index, float x, float y ) {
			((GoldenParticle)emitter.recycle( GoldenParticle.class )).reset( x, y );
		}
	};
	public static final Emitter.Factory UP = new Emitter.Factory() {
		@Override
		public void emit( Emitter emitter, int index, float x, float y ) {
			((Golden.goldParticle)emitter.recycle( Golden.goldParticle.class )).resetUp( x, y );}
	};


	public GoldenParticle() {
		super();
