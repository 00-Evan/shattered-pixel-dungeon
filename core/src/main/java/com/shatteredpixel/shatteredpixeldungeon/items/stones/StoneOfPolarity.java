...
public class StoneOfPolarity extends InventoryStone {
			...
			if (toAugment instanceof Weapon){
				for (final Weapon.Augment aug : Weapon.Augment.values()){
					if (((Weapon) toAugment).augment != aug){
						RedButton btnPSpeed = new RedButton( Messages.get(this, aug.name()) ) {
							@Override
							protected void onClick() {
								hide();
								StoneOfPolarity.this.apply( (Weapon)toAugment, aug );
							}
						};
						btnPSpeed.setRect( MARGIN, pos + MARGIN, BUTTON_WIDTH, BUTTON_HEIGHT );
						add( btnPSpeed );

						pos = btnPSpeed.bottom();
					}
				}
				...
				for (final Armor.Augment aug : Armor.Augment.values()){
					if (((Armor) toAugment).augment != aug){
						RedButton btnPSpeed = new RedButton( Messages.get(this, aug.name()) ) {
							...
							protected void onClick() {
								hide();
								StoneOfPolarity.this.apply( (Armor) toAugment, aug );
							}
						};
						btnPSpeed.setRect( MARGIN, pos + MARGIN, BUTTON_WIDTH, BUTTON_HEIGHT );
						add( btnPSpeed );
