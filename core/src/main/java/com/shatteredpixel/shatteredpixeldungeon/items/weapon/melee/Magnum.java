...
public class Magnum extends MeleeWeapon {
    ...
    public String info() {
        ...
        if (levelKnown) {
            ...
            info += "\n\n" + Messages.get(Magnum.class, "stats_known",
                    ...
                    Bulletmax(Magnum.this.buffedLvl()),
    ...
    public class Bullet extends MissileWeapon {
        ...
        public int damageRoll(Char owner) {
            if (owner.buff(Momentum.class) != null && owner.buff(Momentum.class).freerunning()) {
                return Math.round(Random.NormalIntRange(Bulletmin(Magnum.this.buffedLvl()), Bulletmax(Magnum.this.buffedLvl())) * (1f + 0.15f * ((Hero) owner).pointsInTalent(Talent.PROJECTILE_MOMENTUM)));
            } else {
                return Random.NormalIntRange(Bulletmin(Magnum.this.buffedLvl()), Bulletmax(Magnum.this.buffedLvl()));
            }
        }

        @Override
        public boolean hasEnchant(Class<? extends Enchantment> type, Char owner) {
            return Magnum.this.hasEnchant(type, owner);
        }

        @Override
        public int proc(Char attacker, Char defender, int damage) {
            return Magnum.this.proc(attacker, defender, damage);
        }
        ...
        @Override
        public int STRReq(int lvl) {
            return STRReq(tier, Magnum.this.buffedLvl());
        }
