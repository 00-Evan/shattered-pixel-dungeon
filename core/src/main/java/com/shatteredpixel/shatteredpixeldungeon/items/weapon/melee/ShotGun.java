...
public class ShotGun extends MeleeWeapon {
    ...
    public String info() {
        ...
        if (levelKnown) {
            ...
            info += "\n\n" + Messages.get(ShotGun.class, "stats_unknown",
                    Bulletmin(0),
                    Bulletmax(0),
                    round, max_round, reload_time);
        }
    ...
    public class Bullet extends MissileWeapon {
        ...
        public int damageRoll(Char owner) {
            if (owner.buff(Momentum.class) != null && owner.buff(Momentum.class).freerunning()) {
                return Math.round(Random.NormalIntRange(Bulletmin(ShotGun.this.buffedLvl()), Bulletmax(ShotGun.this.buffedLvl())) * (1f + 0.15f * ((Hero) owner).pointsInTalent(Talent.PROJECTILE_MOMENTUM)));
            } else {
                return Random.NormalIntRange(Bulletmin(ShotGun.this.buffedLvl()), Bulletmax(ShotGun.this.buffedLvl()));
            }
        }

        @Override
        public boolean hasEnchant(Class<? extends Enchantment> type, Char owner) {
            return ShotGun.this.hasEnchant(type, owner);
        }

        @Override
        public int proc(Char attacker, Char defender, int damage) {
            return ShotGun.this.proc(attacker, defender, damage);
        }

        @Override
        public float delayFactor(Char user) {
            return ShotGun.this.delayFactor(user);
        }

        @Override
        public int STRReq(int lvl) {
            return STRReq(tier, ShotGun.this.buffedLvl());
        }
