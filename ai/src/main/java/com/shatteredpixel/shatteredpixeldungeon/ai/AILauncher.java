...
/**
 * This class is mostly copied from the other DesktopLauncher class, maybe more modifications will be needed down the road
 */
public class AILauncher {
	...
	public static void main (String[] args) {
		...
		final String title;
		if (AILauncher.class.getPackage().getSpecificationTitle() == null){
			title = System.getProperty("Specification-Title");
		} else {
			title = AILauncher.class.getPackage().getSpecificationTitle();
		}
		...
		Game.version = AILauncher.class.getPackage().getSpecificationVersion();
		if (Game.version == null) {
			Game.version = System.getProperty("Specification-Version");
		}
		
		try {
			Game.versionCode = Integer.parseInt(AILauncher.class.getPackage().getImplementationVersion());
		...
		String vendor = AILauncher.class.getPackage().getImplementationTitle();
		if (vendor == null) {
		...
		Lwjgl3WindowListener listener = new DesktopWindowListener();
		...
		new Lwjgl3Application(new AIPixelDungeon(new DesktopPlatformSupport()), config);
	}
}
